...
-- ----------------------------
-- Table structure for es_sync_task_config
-- ----------------------------
DROP TABLE IF EXISTS `es_sync_task_config`;
CREATE TABLE `es_sync_task_config` (
  `id` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `task_name` varchar(100) NOT NULL COMMENT '任务名称',
  `description` varchar(500) DEFAULT NULL COMMENT '任务描述',
  `es_endpoint` varchar(255) NOT NULL COMMENT 'ES接口地址',
  `es_index_pattern` varchar(255) NOT NULL DEFAULT 'cluster*:data-zabbix-host-monitor-*' COMMENT 'ES索引模式',
  `cron_expression` varchar(100) NOT NULL COMMENT 'Cron表达式',
  `query_time_range` varchar(20) NOT NULL DEFAULT '30d' COMMENT '查询时间范围(如7d,30d)',
  `is_enabled` tinyint(1) NOT NULL DEFAULT '1' COMMENT '是否启用',
...
) ENGINE=InnoDB AUTO_INCREMENT=2 DEFAULT CHARSET=utf8mb4 COMMENT='ES数据同步任务配置表';

-- ----------------------------
-- Table structure for external_sync_execution_detail
-- ----------------------------
DROP TABLE IF EXISTS `external_sync_execution_detail`;
CREATE TABLE `external_sync_execution_detail` (
  `id` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `execution_id` int(10) unsigned NOT NULL COMMENT '执行记录ID',
  `host_ip` varchar(50) NOT NULL COMMENT '主机IP',
  `host_name` varchar(100) DEFAULT NULL COMMENT '主机名',
  `sync_status` varchar(20) NOT NULL COMMENT '同步状态(success/failed/not_in_pool)',
  `error_message` text COMMENT '错误信息',
  `max_cpu` double DEFAULT NULL COMMENT 'CPU最大使用率',
  `avg_cpu` double DEFAULT NULL COMMENT 'CPU平均使用率',
  `max_memory` double DEFAULT NULL COMMENT '内存最大使用量(GB)',
  `avg_memory` double DEFAULT NULL COMMENT '内存平均使用量(GB)',
  `max_disk` double DEFAULT NULL COMMENT '磁盘最大使用量(GB)',
  `avg_disk` double DEFAULT NULL COMMENT '磁盘平均使用量(GB)',
  `data_point_count` int(11) DEFAULT NULL COMMENT '数据点数量',
  `created_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  PRIMARY KEY (`id`),
  KEY `idx_execution_id` (`execution_id`),
  KEY `idx_host_ip` (`host_ip`),
  KEY `idx_sync_status` (`sync_status`)
) ENGINE=InnoDB AUTO_INCREMENT=1133 DEFAULT CHARSET=utf8mb4 COMMENT='ES数据同步执行详情表';

-- ----------------------------
-- Table structure for external_sync_execution_log
-- ----------------------------
DROP TABLE IF EXISTS `external_sync_execution_log`;
CREATE TABLE `external_sync_execution_log` (
  `id` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `task_id` int(10) unsigned NOT NULL COMMENT '任务配置ID',
  `task_name` varchar(100) NOT NULL COMMENT '任务名称',
  `data_source` enum('elasticsearch','cmsys') NOT NULL DEFAULT 'elasticsearch' COMMENT '数据源类型：elasticsearch-ES数据源，cmsys-CMSys数据源',
  `execution_time` datetime NOT NULL COMMENT '执行时间',
  `execution_status` varchar(20) NOT NULL COMMENT '执行状态(success/failed/partial)',
  `total_hosts` int(11) NOT NULL DEFAULT '0' COMMENT '总主机数',
  `success_count` int(11) NOT NULL DEFAULT '0' COMMENT '成功同步数量',
  `failed_count` int(11) NOT NULL DEFAULT '0' COMMENT '失败同步数量',
  `not_in_pool_count` int(11) NOT NULL DEFAULT '0' COMMENT 'ES有但hosts_pool没有的数量',
  `error_message` text COMMENT '错误信息',
  `duration_ms` int(11) DEFAULT NULL COMMENT '执行时长(毫秒)',
  `query_time_range` varchar(20) DEFAULT NULL COMMENT '查询时间范围',
  `created_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  PRIMARY KEY (`id`),
...
) ENGINE=InnoDB AUTO_INCREMENT=53 DEFAULT CHARSET=utf8mb4 COMMENT='ES数据同步执行记录表';

-- ----------------------------
-- Table structure for external_sync_task_config
-- ----------------------------
DROP TABLE IF EXISTS `external_sync_task_config`;
CREATE TABLE `external_sync_task_config` (
  `id` bigint(20) unsigned NOT NULL AUTO_INCREMENT,
  `task_name` varchar(200) NOT NULL COMMENT '任务名称',
  `description` text COMMENT '任务描述',
  `data_source` enum('elasticsearch','cmsys') NOT NULL DEFAULT 'elasticsearch' COMMENT '数据源类型',
  `es_endpoint` varchar(500) DEFAULT NULL COMMENT 'ES接口地址（可选，有默认值）',
  `es_index_pattern` varchar(200) DEFAULT NULL COMMENT 'ES索引模式（可选，有默认值）',
  `cmsys_query` varchar(1000) DEFAULT NULL COMMENT 'CMSys查询参数（可选）',
  `cron_expression` varchar(100) NOT NULL COMMENT 'Cron表达式',
  `query_time_range` varchar(50) DEFAULT '30d' COMMENT '查询时间范围（如7d, 30d）',
...
) ENGINE=InnoDB AUTO_INCREMENT=4 DEFAULT CHARSET=utf8mb4 COMMENT='统一外部资源同步任务配置表';
...
) ENGINE=InnoDB AUTO_INCREMENT=112 DEFAULT CHARSET=utf8mb4 COMMENT='硬件资源验证执行记录表';
...
CREATE TABLE `hosts_applications` (
  ...
  KEY `idx_pool_cluster` (`pool_id`,`cluster_name`),
  CONSTRAINT `fk_hosts_applications_pool_id` FOREIGN KEY (`pool_id`) REFERENCES `hosts_pool` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB AUTO_INCREMENT=26 DEFAULT CHARSET=utf8mb4 COMMENT='主机应用部署表';
...
CREATE TABLE `hosts_pool` (
  ...
  `update_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  `delete_time` datetime DEFAULT NULL,
  `idc_id` int(10) unsigned DEFAULT NULL COMMENT 'IDC机房ID',
  `remark` varchar(200) DEFAULT NULL COMMENT '机器备注说明',
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_host_ip` (`host_ip`),
  KEY `idx_host_name` (`host_name`),
  KEY `idx_host_type` (`host_type`),
  KEY `idx_is_deleted` (`is_deleted`)
) ENGINE=InnoDB AUTO_INCREMENT=365 DEFAULT CHARSET=utf8mb4 COMMENT='主机资源池表';

-- ----------------------------
-- Table structure for idc_conf
-- ----------------------------
DROP TABLE IF EXISTS `idc_conf`;
CREATE TABLE `idc_conf` (
  `id` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `idc_name` varchar(100) NOT NULL COMMENT 'IDC机房显示名称',
  `idc_code` varchar(50) NOT NULL COMMENT 'IDC机房代码',
  `idc_ip_regexp` varchar(500) NOT NULL COMMENT 'IP正则表达式匹配规则',
  `idc_location` varchar(200) DEFAULT NULL COMMENT 'IDC机房地理位置',
  `idc_description` text COMMENT 'IDC机房描述',
  `is_active` tinyint(1) NOT NULL DEFAULT '1' COMMENT '是否启用',
  `priority` int(10) unsigned NOT NULL DEFAULT '100' COMMENT '匹配优先级(数值越小优先级越高)',
  `created_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `updated_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  `deleted_at` datetime DEFAULT NULL COMMENT '删除时间',
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_idc_code` (`idc_code`),
  KEY `idx_idc_name` (`idc_name`),
  KEY `idx_is_active` (`is_active`),
  KEY `idx_priority` (`priority`),
  KEY `idx_deleted_at` (`deleted_at`)
) ENGINE=InnoDB AUTO_INCREMENT=8 DEFAULT CHARSET=utf8mb4 COMMENT='IDC机房配置表';
...
CREATE TABLE `scheduled_hardware_verification` (
  ...
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_task_name` (`task_name`)
) ENGINE=InnoDB AUTO_INCREMENT=9 DEFAULT CHARSET=utf8mb4 COMMENT='定时硬件资源验证任务配置表';
...
) ENGINE=InnoDB AUTO_INCREMENT=75 DEFAULT CHARSET=utf8mb4 COMMENT='定时任务执行历史表';
...
CREATE TABLE `server_resources` (
  ...
  `mon_date` date DEFAULT NULL COMMENT '监控指标记录日期',
  PRIMARY KEY (`id`),
  UNIQUE KEY `unq_ip_mdate` (`ip`,`mon_date`) USING BTREE,
  KEY `idx_pool_id` (`pool_id`),
  KEY `idx_ip` (`ip`),
  KEY `idx_date_time` (`mon_date`),
  CONSTRAINT `fk_server_resources_pool_id` FOREIGN KEY (`pool_id`) REFERENCES `hosts_pool` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB AUTO_INCREMENT=2223 DEFAULT CHARSET=utf8mb4 COMMENT='服务器资源监控表';
...
) ENGINE=InnoDB AUTO_INCREMENT=131 DEFAULT CHARSET=utf8mb4 COMMENT='用户会话表';
